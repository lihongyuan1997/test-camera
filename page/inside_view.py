...
def choose_palette(wait, driver, palette_name: str) -> None:
    ...
    while i < 2:
        ...
        # 第一次没找到的处理
            ...
            # 找到红头色板
            palette_hongtou: WebElement = get_hongtou_palette(wait)

            # 如果是冰冷、墨灰、白热、黑热色板，没找到的话向右滑
            if palette_name in (BINGLENG, MOHUI, BAIRE, HEIRE):
                swipe_palette_row_to_right(wait, driver, palette_hongtou)

            # 如果是铁红、高彩虹、彩虹、铁灰色板，没找到的话向左滑
            if palette_name in (TIEHONG, GAOCAIHONG, CAIHONG, TIEHUI):
                swipe_palette_row_to_left(wait, driver, palette_hongtou)
